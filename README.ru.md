...
пароль в каждом запросе, поэтому TLS нужен на прокси перед шлюзом либо доверенная сеть.
...
Шлюз работает по обычному HTTP. TLS терминируйте на reverse proxy перед ним — nginx, Caddy,
Traefik, — а сам контейнер держите во внутренней сети.

Встроенный HTTPS существовал до версии 2.0.0 и был убран, а не оставлен ловушкой.
Отладочный сервер Flask оборачивает в TLS **слушающий** сокет, поэтому рукопожатие выполняется
в цикле приёма соединений: один клиент, открывший соединение и замолчавший, блокирует всех
остальных, а браузеры открывают соединения заранее и постоянно. Прокси заодно даёт сертификат,
которому клиенты действительно доверяют.
...
- **Basic Auth передаёт пароль в каждом запросе**, поэтому защищает его именно соединение:
  терминируйте TLS на reverse proxy либо держите шлюз в доверенной сети. Хеш в `USERS` защищает
  место хранения пароля, а не его передачу.
