...
Сгенерировать можно самим образом, ничего доустанавливать не нужно:

```bash
docker run --rm --entrypoint python ghcr.io/xstarina/sms-gammu-gateway:latest \
  -c "from werkzeug.security import generate_password_hash; print(generate_password_hash('ваш-пароль'))"
```

Результат подставляется туда, где был пароль:
...
**В compose-файле каждый `$` удваивается**, как в примере выше: одиночный знак compose считает
началом переменной и молча испортит хеш. В командной строке `docker run` достаточно взять
значение в одинарные кавычки.

Хеши и открытые пароли можно смешивать в одной переменной — хешем считается всё, что начинается
с `scrypt:` или `pbkdf2:`. Проверка намеренно занимает около 70 мс на запрос: именно это делает
подбор по украденному хешу дорогим, а для шлюза, отправляющего десяток сообщений, такая нагрузка
незаметна.
