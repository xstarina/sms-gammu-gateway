...
Пары разделяются запятыми, пробелами или переводами строк, так что блочный скаляр compose с
одной парой на строку читается не хуже. Пароль может содержать двоеточия — разделяет только
первое, — но не запятые и пробелы: они завершают пару.
