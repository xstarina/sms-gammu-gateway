...
Записи, не похожие на адрес или подсеть, пропускаются с предупреждением, а не останавливают
шлюз; если пригодных не осталось, доступ остаётся открытым — опечатка не должна ронять рабочий
сервис. **Проверяйте лог после изменения этой переменной**: молча проигнорированная опечатка
оставляет API доступным отовсюду.
...
- **Пароли передаются открытым текстом** в `USERS`, то есть видны каждому, кто может выполнить
  `docker inspect` на хосте или прочитать compose-файл. Держите этот файл вне системы контроля
  версий, а для более серьёзных требований ставьте перед шлюзом reverse proxy.
